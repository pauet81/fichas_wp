...
function asap_add_index($content)
{
	...
	if ( $matches > 1 )
	{
		try
		{
			...
			if ( ( is_single() && !is_admin() && get_theme_mod('asap_enable_post_index') ) ||
			 	 ( is_page() && !is_admin() && get_theme_mod('asap_enable_page_index') ) ) 
			{
				...
				if ( isset($post->ID) )
				{
					...
					if ( ( $place == 1 || $place == 2 ) && ! $hide_toc )
					{
						...
						if (strpos($content, '[asap_toc]') === false) 
						{
							if ($place == 1)
							{
			...
			{
				if ( isset($post->ID) && strpos($content, '[asap_toc]') !== false )
					{
					    $string = get_the_content($post->ID);
					    $toc = new Toc($string);
					    $result = $toc->getToc();
