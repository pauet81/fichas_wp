...
if (function_exists('is_woocommerce')):
	...
	function asap_wc_scripts() {
		...
		wp_enqueue_style('asap-woocommerce-style', get_template_directory_uri() . '/assets/css/woocommerce.min.css', array() , '02280425');
	...
	function asap_wc_dequeue_styles_scripts() {
		...
		$remove_dependencies = get_option('asap_wc_remove_dependencies');
	...
	function asap_wc_add_cat_editor() {
		...
		add_action('woocommerce_after_shop_loop', 'asap_wc_show_cat_desc', 10);

		function asap_wc_show_cat_desc() {
		    // Solo para taxonomías de producto y sólo si es la primera página
		    if ( is_product_taxonomy() && ! is_paged() ) {
